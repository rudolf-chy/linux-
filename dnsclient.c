...
int dns_client_commit(const char *domain){
    ...
    int slen = sendto(sockfd, request, req_len, 0, (struct sockaddr*)&dest, sizeof(struct sockaddr_in));
...
struct async_ctx *dns_async_client_init(void){
    
    int epfd = epoll_create(1);
    if(epfd < 0) return NULL;

    struct async_ctx *ctx = calloc(1, sizeof(struct async_ctx));
    if(ctx == NULL){
        close(epfd);
        return NULL;
    }
...
int dns_async_client_commit(struct async_ctx *ctx, const char *domain, async_result_cb cb){
...
int main(int argc, char *argv[]){
    ...
    struct async_ctx *ctx = dns_async_client_init();
